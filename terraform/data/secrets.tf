...
resource "random_password" "db_reader_password" {
  keepers = {
    resource_group = azurerm_resource_group.this.name
  }
  length  = 25
...
resource "random_password" "db_writer_password" {
  keepers = {
    resource_group = azurerm_resource_group.this.name
  }
  length  = 25
...
resource "azurerm_key_vault_secret" "db_reader_password" {
  ...
  value        = random_password.db_reader_password.result
  key_vault_id = var.key_vault_id
  tags         = local.tags
}
...
resource "azurerm_key_vault_secret" "db_writer_password" {
  ...
  value        = random_password.db_writer_password.result
  key_vault_id = var.key_vault_id
  tags         = local.tags
}
