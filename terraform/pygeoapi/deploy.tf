...
#echo 'app_url: ${azurerm_service_plan.this.default_site_hostname}' > pygeoapi.secrets
...
# TODO: --resource-group ${var.webapps_resource_group.name}
resource "null_resource" "deploy_zip" {
  ...
  provisioner "local-exec" {
    ...
    az webapp deployment source config-zip --ids ${azurerm_service_plan.this.id} --src ${data.archive_file.deploy.output_path}
